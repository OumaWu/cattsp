...
(function ($) {
    ...
    $.layer = function (deliver) {
        ...
        //适应老版本属性。
        if (deliver.v_closebtn == false) {
            deliver.v_showclose = deliver.v_closebtn;
        }
        if (deliver.v_button) {
            deliver.v_btns = deliver.v_button;
        }
        if (deliver.v_button1) {
            deliver.v_btn[0] = deliver.v_button1;
        }
        if (deliver.v_button2) {
            deliver.v_btn[1] = deliver.v_button2;
        }
        if (deliver.v_top != null) {
            deliver.v_offset[0] = deliver.v_top + 'px';
        }
        if (deliver.v_width != null) {
            deliver.v_area[0] = deliver.v_width + 'px';
        }
        if (deliver.v_height != null) {
            deliver.v_area[1] = deliver.v_height + 'px';
        }
        if (deliver.v_boxdom) {
            deliver.v_dom = deliver.v_boxdom;
        }
        ...
        if (deliver.v_box == 0) {
            ...
            if (H.i == 0) {
                $.layer({v_title: '插件警告', v_msg: 'layer在输出时发现不应该的程序错误，请检查! '});
            }
            c = 0;
            layer_close();
            $("body").append(shade);
        ...
        $xubox_layer.css({
            'top': deliver.v_offset[0],
            'left': _left,
            'width': deliver.v_area[0],
            'height': deliver.v_area[1],
            'margin-left': -_mleft
        });
        ...
        deliver = $.extend({
            ...
            yes: function () {
                ...
                // 获得焦点输入框的ID
                var focusId = deliver.v_focus;
                if (undefined != focusId && null != focusId && focusId != "") {
                    if (focusId.substring(0, 1) != "#") {
                        focusId = "#" + focusId;
                    }
                    $(focusId).focus();
                }
                // 如果是弹出选择框，点确定的时候，默认就提交掉吧
                if (deliver.v_btns == 2) {
                    $("#gform").submit();
                }
            },
 ...
 * 弹出层询问框方法
 * msg    -- 提示语句
 * yesCallback    -- 点确定后的回调函数（默认是关闭弹出框，提交gform）
 * noCallback    -- 点取消后的回调函数（默认是关闭弹出框）
 * width            -- 提示框宽度px（默认310px）
 * height        -- 提示框高度px（默认200px）
 */
function gConfirm(msg, yesCallback, noCallback, width, height) {
 ...
 * height        -- 提示框高度px（默认200px）
 */
function shoppingDialog(msg, btn, yesCallback, noCallback, width, height) {
    if (undefined == width || null == width || width == "") {
        width = "310px";
    }
    if (undefined == height || null == height || height == "") {
